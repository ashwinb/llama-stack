...
# Dynamically load required modules without importing llama_stack package __init__
ROOT = pathlib.Path(__file__).resolve().parent.parent

# Load preserve_contexts_async_generator from its file path
context_spec = importlib.util.spec_from_file_location(
    "context_utils", ROOT / "llama_stack" / "core" / "utils" / "context.py"
)
context_mod = importlib.util.module_from_spec(context_spec)
context_spec.loader.exec_module(context_mod)
preserve_contexts_async_generator = context_mod.preserve_contexts_async_generator
