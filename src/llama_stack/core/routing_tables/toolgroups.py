...
class ToolGroupsRoutingTable(CommonRoutingTableImpl, ToolGroups):
    ...
    async def list_tools(self, request: ListToolsRequest, authorization: str | None = None) -> ListToolDefsResponse:
        ...
        for toolgroup in toolgroups:
            if toolgroup.identifier not in self.toolgroups_to_tools:
                try:
                    await self._index_tools(toolgroup, authorization=authorization)  # ty: ignore[invalid-argument-type]  # toolgroup is ToolGroup at runtime
    ...
    async def list_tool_groups(self) -> ListToolGroupsResponse:
        return ListToolGroupsResponse(data=await self.get_all_with_type("tool_group"))  # ty: ignore[invalid-argument-type]  # registry returns union but all are ToolGroups at runtime
    ...
    async def get_tool_group(self, toolgroup_id: str) -> ToolGroup:
        ...
        return tool_group  # ty: ignore[invalid-return-type]  # get_object_by_identifier returns union but filtered to ToolGroup at runtime
    ...
    async def unregister_toolgroup(self, toolgroup_id: str) -> None:
        await self.unregister_object(await self.get_tool_group(toolgroup_id))  # ty: ignore[invalid-argument-type]  # ToolGroup satisfies RoutableObjectWithProvider at runtime
