...
class ToolGroupsRoutingTable(CommonRoutingTableImpl, ToolGroups):
    ...
    async def list_tools(self, request: ListToolsRequest, authorization: str | None = None) -> ListToolDefsResponse:
        ...
        for toolgroup in toolgroups:
            if toolgroup.identifier not in self.toolgroups_to_tools:
                try:
                    await self._index_tools(toolgroup, authorization=authorization)  # ty: ignore[invalid-argument-type] # RoutableObjectWithProvider union vs ToolGroup
    ...
    async def list_tool_groups(self) -> ListToolGroupsResponse:
        return ListToolGroupsResponse(data=await self.get_all_with_type("tool_group"))  # ty: ignore[invalid-argument-type] # RoutableObjectWithProvider union vs ToolGroup
    ...
    async def get_tool_group(self, toolgroup_id: str) -> ToolGroup:
        ...
        return tool_group  # ty: ignore[invalid-return-type] # RoutableObjectWithProvider union vs ToolGroup
    ...
    async def unregister_toolgroup(self, toolgroup_id: str) -> None:
        await self.unregister_object(await self.get_tool_group(toolgroup_id))  # ty: ignore[invalid-argument-type] # ToolGroup vs RoutableObjectWithProvider union
