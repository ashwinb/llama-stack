...
class BenchmarksRoutingTable(CommonRoutingTableImpl, Benchmarks):
    ...
    async def list_benchmarks(self, request: ListBenchmarksRequest) -> ListBenchmarksResponse:
        return ListBenchmarksResponse(data=await self.get_all_with_type("benchmark"))  # ty: ignore[invalid-argument-type]  # registry returns union but all are Benchmarks at runtime
    ...
    async def get_benchmark(self, request: GetBenchmarkRequest) -> Benchmark:
        ...
        return benchmark  # ty: ignore[invalid-return-type]  # get_object_by_identifier returns union but filtered to Benchmark at runtime
    ...
    async def unregister_benchmark(self, request: UnregisterBenchmarkRequest) -> None:
        ...
        await self.unregister_object(existing_benchmark)  # ty: ignore[invalid-argument-type]  # Benchmark satisfies RoutableObjectWithProvider at runtime
