...
class ModelsRoutingTable(CommonRoutingTableImpl, Models):
    ...
    async def refresh(self) -> None:
        for provider_id, provider in self.impls_by_provider_id.items():
            refresh = await provider.should_refresh_models()  # ty: ignore[unresolved-attribute]  # provider is Inference impl at runtime
            refresh = refresh or provider_id not in self.listed_providers
            if not refresh:
                continue

            try:
                models = await provider.list_models()  # ty: ignore[unresolved-attribute]  # provider is Inference impl at runtime
        ...
        for provider_id, provider in self.impls_by_provider_id.items():
            ...
            try:
                ...
                for model in models:
                    ...
                    if is_action_allowed(self.policy, "read", temp_model, user):  # ty: ignore[invalid-argument-type]  # ModelWithOwner satisfies ProtectedResource protocol at runtime
    ...
    async def list_models(self) -> ListModelsResponse:
        ...
        return ListModelsResponse(data=registry_models + unique_dynamic_models)  # ty: ignore[invalid-argument-type]  # registry returns union but all are Models at runtime
    ...
    async def openai_list_models(self) -> OpenAIListModelsResponse:
        ...
        openai_models = [
            OpenAIModel(
                ...
                custom_metadata={
                    "model_type": model.model_type,  # ty: ignore[unresolved-attribute]  # model is Model at runtime, has model_type
                    "provider_id": model.provider_id,
                    "provider_resource_id": model.provider_resource_id,
                    **model.metadata,  # ty: ignore[unresolved-attribute]  # model is Model at runtime, has metadata
                },
            )
    ...
    async def get_model(self, request_or_model_id: GetModelRequest | str) -> Model:  # ty: ignore[invalid-method-override]  # widened param type to support both API and internal ModelStore interface
    ...
    async def get_provider_impl(self, model_id: str) -> Any:  # ty: ignore[invalid-method-override]  # simplified signature for model-specific lookup
    ...
    async def register_model(
        ...
        *,
        ...
        return registered_model  # ty: ignore[invalid-return-type]  # register_object returns union but is Model at runtime
    ...
    async def unregister_model(
        ...
        *,
        ...
        await self.unregister_object(existing_model)  # ty: ignore[invalid-argument-type]  # Model satisfies RoutableObjectWithProvider at runtime
