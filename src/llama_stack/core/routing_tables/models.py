...
class ModelsRoutingTable(CommonRoutingTableImpl, Models):
    ...
    async def refresh(self) -> None:
        for provider_id, provider in self.impls_by_provider_id.items():
            refresh = await provider.should_refresh_models()  # ty: ignore[unresolved-attribute]  # runtime dispatch to correct provider
            refresh = refresh or provider_id not in self.listed_providers
            if not refresh:
                continue

            try:
                models = await provider.list_models()  # ty: ignore[unresolved-attribute]  # runtime dispatch to correct provider
        ...
        for provider_id, provider in self.impls_by_provider_id.items():
            ...
            try:
                ...
                for model in models:
                    ...
                    if is_action_allowed(self.policy, "read", temp_model, user):  # ty: ignore[invalid-argument-type]  # runtime dispatch handles union types
    ...
    async def list_models(self) -> ListModelsResponse:
        ...
        return ListModelsResponse(data=registry_models + unique_dynamic_models)  # ty: ignore[invalid-argument-type]  # concrete type accepted at runtime
    ...
    async def openai_list_models(self) -> OpenAIListModelsResponse:
        ...
        openai_models = [
            OpenAIModel(
                ...
                custom_metadata={
                    "model_type": model.model_type,  # ty: ignore[unresolved-attribute]  # available on concrete routing table impl
                    "provider_id": model.provider_id,
                    "provider_resource_id": model.provider_resource_id,
                    **model.metadata,  # ty: ignore[unresolved-attribute]  # available on concrete routing table impl
                },
            )
    ...
    async def get_model(self, request_or_model_id: GetModelRequest | str) -> Model:  # ty: ignore[invalid-method-override]  # intentionally narrowed override
    ...
    async def get_provider_impl(self, model_id: str) -> RoutedProtocol:  # type: ignore[override]  # ty: ignore[invalid-method-override]  # intentionally narrowed override
    ...
    async def register_model(
        ...
        *,
        ...
        return registered_model  # ty: ignore[invalid-return-type]  # returns narrower type than parent
    ...
    async def unregister_model(
        ...
        *,
        ...
        await self.unregister_object(existing_model)  # ty: ignore[invalid-argument-type]  # unregister_object accepts RoutableObjectWithProvider
