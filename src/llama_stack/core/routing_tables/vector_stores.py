...
class VectorStoresRoutingTable(CommonRoutingTableImpl):
    ...
    async def list_vector_stores(self) -> list[VectorStoreWithOwner]:
        ...
        return await self.get_all_with_type(ResourceType.vector_store.value)  # ty: ignore[invalid-return-type]  # registry returns union but all are VectorStoreWithOwner at runtime
    ...
    async def register_vector_store(
        ...
        vector_store = VectorStoreWithOwner(
            ...
            type=ResourceType.vector_store.value,  # ty: ignore[invalid-argument-type]  # string value matches expected Literal at runtime
            provider_id=provider_id,
            provider_resource_id=provider_vector_store_id,
            embedding_model=embedding_model,
            embedding_dimension=embedding_dimension,  # ty: ignore[invalid-argument-type]  # embedding_dimension defaults to 384, None handled by model
    ...
    async def insert_chunks(
        ...
        await self.assert_action_allowed("update", "vector_store", request.vector_store_id)  # ty: ignore[invalid-argument-type]  # string literal is valid Action at runtime
    ...
    async def query_chunks(
        ...
        await self.assert_action_allowed("read", "vector_store", request.vector_store_id)  # ty: ignore[invalid-argument-type]  # string literal is valid Action at runtime
        provider = await self.get_provider_impl(request.vector_store_id)
    ...
    async def openai_retrieve_vector_store(
        ...
        await self.assert_action_allowed("read", "vector_store", vector_store_id)  # ty: ignore[invalid-argument-type]  # string literal is valid Action at runtime
    ...
    async def openai_update_vector_store(
        ...
        await self.assert_action_allowed("update", "vector_store", vector_store_id)  # ty: ignore[invalid-argument-type]  # string literal is valid Action at runtime
        provider = await self.get_provider_impl(vector_store_id)
    ...
    async def openai_delete_vector_store(
        ...
        await self.assert_action_allowed("delete", "vector_store", vector_store_id)  # ty: ignore[invalid-argument-type]  # string literal is valid Action at runtime
        provider = await self.get_provider_impl(vector_store_id)
