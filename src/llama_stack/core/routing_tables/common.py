...
class CommonRoutingTableImpl(RoutingTable):
    ...
    async def initialize(self) -> None:
        ...
        for pid, p in self.impls_by_provider_id.items():
            api = get_impl_api(p)
            if api == Api.inference:
                p.model_store = self  # ty: ignore[invalid-assignment] # dynamic provider attribute
            elif api == Api.safety:
                p.shield_store = self  # ty: ignore[invalid-assignment] # dynamic provider attribute
            elif api == Api.vector_io:
                p.vector_store_store = self  # ty: ignore[invalid-assignment] # dynamic provider attribute
            elif api == Api.datasetio:
                p.dataset_store = self  # ty: ignore[invalid-assignment] # dynamic provider attribute
            elif api == Api.scoring:
                p.scoring_function_store = self  # ty: ignore[invalid-assignment] # dynamic provider attribute
                ...
                p.benchmark_store = self  # ty: ignore[invalid-assignment] # dynamic provider attribute
            elif api == Api.tool_runtime:
                p.tool_store = self  # ty: ignore[invalid-assignment] # dynamic provider attribute

    async def shutdown(self) -> None:
        for p in self.impls_by_provider_id.values():
            await p.shutdown()  # ty: ignore[unresolved-attribute] # shutdown exists on all providers
    ...
    async def get_object_by_identifier(self, type: str, identifier: str) -> RoutableObjectWithProvider | None:
        ...
        if not is_action_allowed(self.policy, "read", obj, get_authenticated_user()):  # ty: ignore[invalid-argument-type] # Action/ProtectedResource union mismatch
    ...
    async def unregister_object(self, obj: RoutableObjectWithProvider) -> None:
        ...
        if not is_action_allowed(self.policy, "delete", obj, user):  # ty: ignore[invalid-argument-type] # Action/ProtectedResource union mismatch
            raise AccessDeniedError("delete", obj, user)  # ty: ignore[invalid-argument-type] # ProtectedResource union mismatch
    ...
    async def register_object(self, obj: RoutableObjectWithProvider) -> RoutableObjectWithProvider:
        ...
        if not is_action_allowed(self.policy, "create", obj, creator):  # ty: ignore[invalid-argument-type] # Action/ProtectedResource union mismatch
            raise AccessDeniedError("create", obj, creator)  # ty: ignore[invalid-argument-type] # ProtectedResource union mismatch
        if creator:
            obj.owner = creator
        ...
        if obj.type == ResourceType.vector_store.value:
            if hasattr(p, "_ensure_openai_metadata_exists"):
                await p._ensure_openai_metadata_exists(obj)  # ty: ignore[call-non-callable] # guarded by hasattr check
            else:
                logger.warning(
        ...
        if obj.type == ResourceType.model.value:
            await self.dist_registry.register(registered_obj)  # ty: ignore[invalid-argument-type] # RoutableObject vs RoutableObjectWithProvider
            return registered_obj  # ty: ignore[invalid-return-type] # RoutableObject vs RoutableObjectWithProvider
        else:
            await self.dist_registry.register(obj)
            return obj
    ...
    async def assert_action_allowed(
        ...
        user = get_authenticated_user()
        if not is_action_allowed(self.policy, action, obj, user):  # ty: ignore[invalid-argument-type] # ProtectedResource union mismatch
            raise AccessDeniedError(action, obj, user)  # ty: ignore[invalid-argument-type] # ProtectedResource union mismatch
    ...
    async def get_all_with_type(self, type: str) -> list[RoutableObjectWithProvider]:
        ...
        if filtered_objs:
            filtered_objs = [
                ...
                if is_action_allowed(self.policy, "read", obj, get_authenticated_user())  # ty: ignore[invalid-argument-type] # Action/ProtectedResource union mismatch
            ]
...
async def lookup_model(routing_table: CommonRoutingTableImpl, model_id: str) -> Model:
    ...
    return model  # ty: ignore[invalid-return-type] # RoutableObjectWithProvider union vs Model
