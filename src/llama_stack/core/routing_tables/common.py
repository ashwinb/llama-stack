...
class CommonRoutingTableImpl(RoutingTable):
    ...
    async def initialize(self) -> None:
        ...
        for pid, p in self.impls_by_provider_id.items():
            api = get_impl_api(p)
            if api == Api.inference:
                p.model_store = self  # ty: ignore[invalid-assignment]  # runtime-injected store attribute
            elif api == Api.safety:
                p.shield_store = self  # ty: ignore[invalid-assignment]  # runtime-injected store attribute
            elif api == Api.vector_io:
                p.vector_store_store = self  # ty: ignore[invalid-assignment]  # runtime-injected store attribute
            elif api == Api.datasetio:
                p.dataset_store = self  # ty: ignore[invalid-assignment]  # runtime-injected store attribute
            elif api == Api.scoring:
                p.scoring_function_store = self  # ty: ignore[invalid-assignment]  # runtime-injected store attribute
                ...
                p.benchmark_store = self  # ty: ignore[invalid-assignment]  # runtime-injected store attribute
            elif api == Api.tool_runtime:
                p.tool_store = self  # ty: ignore[invalid-assignment]  # runtime-injected store attribute
