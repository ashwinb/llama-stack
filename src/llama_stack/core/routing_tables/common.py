...
class CommonRoutingTableImpl(RoutingTable):
    ...
    async def initialize(self) -> None:
        ...
        for pid, p in self.impls_by_provider_id.items():
            ...
            if api == Api.inference:
                ...
                scoring_functions = await p.list_scoring_functions()  # ty: ignore[unresolved-attribute]  # p is Scoring provider at runtime
    ...
    async def get_object_by_identifier(self, type: str, identifier: str) -> RoutableObjectWithProvider | None:
        ...
        if not is_action_allowed(self.policy, "read", obj, get_authenticated_user()):  # ty: ignore[invalid-argument-type]  # obj satisfies ProtectedResource protocol at runtime
    ...
    async def unregister_object(self, obj: RoutableObjectWithProvider) -> None:
        ...
        if not is_action_allowed(self.policy, "delete", obj, user):  # ty: ignore[invalid-argument-type]  # obj satisfies ProtectedResource protocol at runtime
            raise AccessDeniedError("delete", obj, user)  # ty: ignore[invalid-argument-type]  # obj satisfies ProtectedResource protocol at runtime
    ...
    async def register_object(self, obj: RoutableObjectWithProvider) -> RoutableObjectWithProvider:
        ...
        if not is_action_allowed(self.policy, "create", obj, creator):  # ty: ignore[invalid-argument-type]  # obj satisfies ProtectedResource protocol at runtime
            raise AccessDeniedError("create", obj, creator)  # ty: ignore[invalid-argument-type]  # obj satisfies ProtectedResource protocol at runtime
        if creator:
            obj.owner = creator
        ...
        if obj.type == ResourceType.vector_store.value:
            if hasattr(p, "_ensure_openai_metadata_exists"):
                await p._ensure_openai_metadata_exists(obj)  # ty: ignore[call-non-callable]  # verified by hasattr check above
            else:
                logger.warning(
        ...
        if obj.type == ResourceType.model.value:
            await self.dist_registry.register(registered_obj)  # ty: ignore[invalid-argument-type]  # registered_obj is RoutableObject returned by provider
            return registered_obj  # ty: ignore[invalid-return-type]  # registered_obj is RoutableObjectWithProvider at runtime
        else:
            await self.dist_registry.register(obj)
            return obj
    ...
    async def assert_action_allowed(
        ...
        user = get_authenticated_user()
        if not is_action_allowed(self.policy, action, obj, user):  # ty: ignore[invalid-argument-type]  # obj satisfies ProtectedResource protocol at runtime
            raise AccessDeniedError(action, obj, user)  # ty: ignore[invalid-argument-type]  # obj satisfies ProtectedResource protocol at runtime
    ...
    async def get_all_with_type(self, type: str) -> list[RoutableObjectWithProvider]:
        ...
        if filtered_objs:
            filtered_objs = [
                ...
                if is_action_allowed(self.policy, "read", obj, get_authenticated_user())  # ty: ignore[invalid-argument-type]  # obj satisfies ProtectedResource protocol at runtime
            ]
...
async def lookup_model(routing_table: CommonRoutingTableImpl, model_id: str) -> Model:
    ...
    return model  # ty: ignore[invalid-return-type]  # get_object_by_identifier returns union but filtered to Model at runtime
