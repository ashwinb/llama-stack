...
class CommonRoutingTableImpl(RoutingTable):
    ...
    async def initialize(self) -> None:
        ...
        for pid, p in self.impls_by_provider_id.items():
            api = get_impl_api(p)
            if api == Api.inference:
                p.model_store = self  # type: ignore[attr-defined]  # ty:ignore[invalid-assignment]  # runtime-injected
            elif api == Api.safety:
                p.shield_store = self  # type: ignore[attr-defined]  # ty:ignore[invalid-assignment]  # runtime-injected
            elif api == Api.vector_io:
                p.vector_store_store = self  # type: ignore[attr-defined]  # ty:ignore[invalid-assignment]  # runtime-injected
            elif api == Api.datasetio:
                p.dataset_store = self  # type: ignore[attr-defined]  # ty:ignore[invalid-assignment]  # runtime-injected
            elif api == Api.scoring:
                p.scoring_function_store = self  # type: ignore[attr-defined]  # ty:ignore[invalid-assignment]  # runtime-injected
                scoring_functions = await p.list_scoring_functions()  # type: ignore[attr-defined]  # ty:ignore[unresolved-attribute]  # runtime-injected
                await add_objects(scoring_functions, pid, ScoringFnWithOwner)
            elif api == Api.eval:
                p.benchmark_store = self  # type: ignore[attr-defined]  # ty:ignore[invalid-assignment]  # runtime-injected
            elif api == Api.tool_runtime:
                p.tool_store = self  # type: ignore[attr-defined]  # ty:ignore[invalid-assignment]  # runtime-injected
