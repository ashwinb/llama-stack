...
class DatasetsRoutingTable(CommonRoutingTableImpl, Datasets):
    ...
    async def list_datasets(self) -> ListDatasetsResponse:
        return ListDatasetsResponse(data=await self.get_all_with_type(ResourceType.dataset.value))  # ty: ignore[invalid-argument-type] # RoutableObjectWithProvider union vs Dataset
    ...
    async def get_dataset(self, request: GetDatasetRequest) -> Dataset:
        ...
        return dataset  # ty: ignore[invalid-return-type] # RoutableObjectWithProvider union vs Dataset
    ...
    async def register_dataset(self, request: RegisterDatasetRequest) -> Dataset:
        ...
        if isinstance(source, dict):
            if source["type"] == "uri":
                source = URIDataSource.parse_obj(source)  # ty: ignore[deprecated] # legacy parse_obj usage
            elif source["type"] == "rows":
                source = RowsDataSource.parse_obj(source)  # ty: ignore[deprecated] # legacy parse_obj usage
        ...
        if metadata and metadata.get("provider_id"):
            ...
            if source.uri.startswith("huggingface"):  # ty: ignore[unresolved-attribute] # narrowed by type check above
        ...
        dataset = DatasetWithOwner(
            ...
            provider_id=provider_id,  # ty: ignore[invalid-argument-type] # provider_id may be None but is validated above
            purpose=purpose,
            source=source,
            metadata=metadata,
        )
    ...
    async def unregister_dataset(self, request: UnregisterDatasetRequest) -> None:
        ...
        await self.unregister_object(dataset)  # ty: ignore[invalid-argument-type] # Dataset vs RoutableObjectWithProvider union
