...
class DatasetsRoutingTable(CommonRoutingTableImpl, Datasets):
    ...
    async def list_datasets(self) -> ListDatasetsResponse:
        return ListDatasetsResponse(data=await self.get_all_with_type(ResourceType.dataset.value))  # ty: ignore[invalid-argument-type]  # registry returns union but all are Datasets at runtime
    ...
    async def get_dataset(self, request: GetDatasetRequest) -> Dataset:
        ...
        return dataset  # ty: ignore[invalid-return-type]  # get_object_by_identifier returns union but filtered to Dataset at runtime
    ...
    async def register_dataset(self, request: RegisterDatasetRequest) -> Dataset:
        ...
        if metadata and metadata.get("provider_id"):
            ...
            if source.uri.startswith("huggingface"):  # ty: ignore[unresolved-attribute]  # guarded by source.type == uri check above
        ...
        dataset = DatasetWithOwner(
            ...
            provider_id=provider_id,  # ty: ignore[invalid-argument-type]  # provider_id is narrowed to str by control flow above
            purpose=purpose,
            source=source,
            metadata=metadata,
        )
    ...
    async def unregister_dataset(self, request: UnregisterDatasetRequest) -> None:
        ...
        await self.unregister_object(dataset)  # ty: ignore[invalid-argument-type]  # Dataset satisfies RoutableObjectWithProvider at runtime
