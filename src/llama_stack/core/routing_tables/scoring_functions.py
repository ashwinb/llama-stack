...
class ScoringFunctionsRoutingTable(CommonRoutingTableImpl, ScoringFunctions):
    ...
    async def list_scoring_functions(self, request: ListScoringFunctionsRequest) -> ListScoringFunctionsResponse:
        return ListScoringFunctionsResponse(data=await self.get_all_with_type(ResourceType.scoring_function.value))  # ty: ignore[invalid-argument-type]  # concrete type accepted at runtime
    ...
    async def get_scoring_function(self, request: GetScoringFunctionRequest) -> ScoringFn:
        ...
        return scoring_fn  # ty: ignore[invalid-return-type]  # returns narrower type than parent
    ...
    async def unregister_scoring_function(self, request: UnregisterScoringFunctionRequest) -> None:
        ...
        await self.unregister_object(existing_scoring_fn)  # ty: ignore[invalid-argument-type]  # unregister_object accepts RoutableObjectWithProvider
