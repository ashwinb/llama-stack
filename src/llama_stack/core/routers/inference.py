...
class InferenceRouter(Inference):
    ...
    async def _get_provider_by_fallback(self, model_id: str, expected_model_type: str) -> tuple[Inference, str]:
        ...
        temp_model = ModelWithOwner(
            ...
            model_type=expected_model_type,  # ty: ignore[invalid-argument-type]  # ModelType enum value accepted at runtime
            metadata={},  # Empty metadata for temporary object
        )

        # Perform RBAC check
        user = get_authenticated_user()
        if not is_action_allowed(self.routing_table.policy, "read", temp_model, user):  # ty: ignore[unresolved-attribute, invalid-argument-type]  # policy attr and types available on concrete impl
            logger.debug(
    ...
    async def rerank(  # ty: ignore[invalid-method-override]  # override uses concrete RerankRequest
        self,
    ...
    async def openai_completion(
        ...
        if params.stream:
            return await provider.openai_completion(params)  # ty: ignore[invalid-return-type]  # streaming path returns compatible type

        response = await provider.openai_completion(params)
        assert isinstance(response, OpenAICompletion), "Expected non-streaming response"
    ...
    async def openai_chat_completion(
        ...
        if params.stream:
            ...
            return self.stream_tokens_and_compute_metrics_openai_chat(
                response=response_stream,  # ty: ignore[invalid-argument-type]  # streaming path guarantees AsyncIterator
                fully_qualified_model_id=request_model_id,
                provider_id=provider.__provider_id__,  # ty: ignore[unresolved-attribute]  # set dynamically on provider instances
                messages=params.messages,
            )
            ...
            if id and self.store and messages:
                ...
                for choice_idx, choice_data in choices_data.items():
                    ...
                    message = OpenAIChatCompletionResponseMessage(
                        ...
                        tool_calls=assembled_tool_calls if assembled_tool_calls else None,  # ty: ignore[invalid-argument-type]  # list[ToolCall] is compatible with expected union type
                    )
