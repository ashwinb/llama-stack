...
class InferenceRouter(Inference):
    ...
    async def register_model(
        ...
        await self.routing_table.register_model(request)  # ty: ignore[unresolved-attribute]  # available on concrete routing table impl

    async def _get_model_provider(self, model_id: str, expected_model_type: str) -> tuple[Inference, str]:
        model = await self.routing_table.get_object_by_identifier("model", model_id)  # ty: ignore[unresolved-attribute]  # available on concrete routing table impl
        if model:
    ...
    async def _get_provider_by_fallback(self, model_id: str, expected_model_type: str) -> tuple[Inference, str]:
        ...
        if provider_id not in self.routing_table.impls_by_provider_id:  # ty: ignore[unresolved-attribute]  # available on concrete routing table impl
        ...
        temp_model = ModelWithOwner(
            ...
            model_type=expected_model_type,  # ty: ignore[invalid-argument-type]  # concrete type accepted at runtime
            metadata={},  # Empty metadata for temporary object
        )

        # Perform RBAC check
        user = get_authenticated_user()
        if not is_action_allowed(self.routing_table.policy, "read", temp_model, user):  # ty: ignore[unresolved-attribute]  # available on concrete routing table impl  # ty: ignore[invalid-argument-type]  # runtime dispatch handles union types
            logger.debug(
        ...
        return self.routing_table.impls_by_provider_id[provider_id], provider_resource_id  # ty: ignore[unresolved-attribute]  # available on concrete routing table impl

    async def rerank(  # ty: ignore[invalid-method-override]  # intentionally narrowed override
        self,
    ...
    async def openai_completion(
        ...
        if params.stream:
            return await provider.openai_completion(params)  # ty: ignore[invalid-return-type]  # returns narrower type than parent

        response = await provider.openai_completion(params)
        response.model = request_model_id  # ty: ignore[invalid-assignment]  # non-streaming response
        return response  # ty: ignore[invalid-return-type]  # returns narrower type than parent
    ...
    async def openai_chat_completion(
        ...
        if params.stream:
            ...
            return self.stream_tokens_and_compute_metrics_openai_chat(
                response=response_stream,  # ty: ignore[invalid-argument-type]  # streaming response type
                fully_qualified_model_id=request_model_id,
                provider_id=provider.__provider_id__,  # ty: ignore[unresolved-attribute]  # runtime dispatch to correct provider
                messages=params.messages,
            )
    ...
    async def _nonstream_openai_chat_completion(
        ...
        for choice in response.choices:  # ty: ignore[unresolved-attribute]  # non-streaming response
            ...
            if id and self.store and messages:
                ...
                for choice_idx, choice_data in choices_data.items():
                    ...
                    message = OpenAIChatCompletionResponseMessage(
                        ...
                        tool_calls=assembled_tool_calls if assembled_tool_calls else None,  # ty: ignore[invalid-argument-type]  # concrete type accepted at runtime
                    )
