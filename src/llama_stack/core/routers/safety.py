...
class SafetyRouter(Safety):
    ...
    async def register_shield(self, request: RegisterShieldRequest) -> Shield:
        ...
        return await self.routing_table.register_shield(request)  # ty: ignore[unresolved-attribute]  # available on concrete routing table impl

    async def unregister_shield(self, identifier: str) -> None:
        logger.debug("SafetyRouter.unregister_shield", identifier=identifier)
        return await self.routing_table.unregister_shield(UnregisterShieldRequest(identifier=identifier))  # ty: ignore[unresolved-attribute]  # available on concrete routing table impl
    ...
    async def run_moderation(self, request: RunModerationRequest) -> ModerationObject:
        list_shields_response = await self.routing_table.list_shields()  # ty: ignore[unresolved-attribute]  # available on concrete routing table impl
