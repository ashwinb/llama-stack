...
class VectorIORouter(VectorIO):
    ...
    def _get_provider_id(self, vector_store_id: str) -> str:
        ...
        """
        try:
            obj = self.routing_table.dist_registry.get_cached("vector_store", vector_store_id)  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
            if obj is None:
    ...
    async def _rewrite_query_for_search(self, query: str) -> str:
        ...
        try:
            ...
            content = response.choices[0].message.content  # ty: ignore[unresolved-attribute] - response is OpenAIChatCompletion in non-stream path
            if content is None:
    ...
    async def _get_embedding_model_dimension(self, embedding_model_id: str) -> int:
        ...
        all_models = await self.routing_table.get_all_with_type("model")  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
    ...
    async def insert_chunks(
        ...
        try:
            result = await self.routing_table.insert_chunks(request)  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
    ...
    async def query_chunks(
        ...
        try:
            ...
            if not request.params or "filters" not in request.params:
                result = await self.routing_table.query_chunks(request)  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
            else:
                ...
                result = await self.routing_table.query_chunks(modified_request)  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
    ...
    async def openai_create_vector_store(
        ...
        if embedding_model is not None:
            model = await self.routing_table.get_object_by_identifier("model", embedding_model)  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
            if model is None:
        ...
        if provider_id is None:
            num_providers = len(self.routing_table.impls_by_provider_id)  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
            if num_providers == 0:
                raise ValueError("No vector_io providers available")
            if num_providers > 1:
                available_providers = list(self.routing_table.impls_by_provider_id.keys())  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
                ...
                provider_id = list(self.routing_table.impls_by_provider_id.keys())[0]  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime

        vector_store_id = f"vs_{uuid.uuid4()}"
        registered_vector_store = await self.routing_table.register_vector_store(  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
    ...
    async def openai_list_vector_stores(
        ...
        vector_stores = await self.routing_table.get_all_with_type("vector_store")  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
        all_stores = []
        for vector_store in vector_stores:
            try:
                vector_store_obj = await self.routing_table.openai_retrieve_vector_store(vector_store.identifier)  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
        ...
        has_more = len(all_stores) > limit  # ty: ignore[unsupported-operator] - limit has a default value of 20, not None at runtime
    ...
    async def openai_retrieve_vector_store(
        ...
        return await self.routing_table.openai_retrieve_vector_store(vector_store_id)  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
    ...
    async def openai_update_vector_store(
        ...
        if request.metadata and "provider_id" in request.metadata:
            current_store = await self.routing_table.get_object_by_identifier("vector_store", vector_store_id)  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
        ...
        return await self.routing_table.openai_update_vector_store(  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
            vector_store_id=vector_store_id,
            request=request,
        )
    ...
    async def openai_delete_vector_store(
        ...
        try:
            result = await self.routing_table.openai_delete_vector_store(vector_store_id)  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
    ...
    async def openai_search_vector_store(
        ...
        try:
            ...
            result = await self.routing_table.openai_search_vector_store(  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
                vector_store_id=vector_store_id,
    ...
    async def openai_attach_file_to_vector_store(
        ...
        try:
            result = await self.routing_table.openai_attach_file_to_vector_store(  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
                vector_store_id=vector_store_id,
                request=params,
            )
            duration = time.perf_counter() - start_time
    ...
    async def openai_list_files_in_vector_store(
        ...
        return await self.routing_table.openai_list_files_in_vector_store(  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
            vector_store_id=vector_store_id,
            limit=limit,
            order=order,
            after=after,
    ...
    async def openai_retrieve_vector_store_file(
        ...
        return await self.routing_table.openai_retrieve_vector_store_file(  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
            vector_store_id=vector_store_id,
            file_id=file_id,
        )
    ...
    async def openai_retrieve_vector_store_file_contents(
        ...
        return await self.routing_table.openai_retrieve_vector_store_file_contents(  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
            vector_store_id=vector_store_id,
            file_id=file_id,
            include_embeddings=include_embeddings,
            include_metadata=include_metadata,
    ...
    async def openai_update_vector_store_file(
        ...
        return await self.routing_table.openai_update_vector_store_file(  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
            vector_store_id=vector_store_id,
            file_id=file_id,
            request=request,
        )
    ...
    async def openai_delete_vector_store_file(
        ...
        try:
            result = await self.routing_table.openai_delete_vector_store_file(  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
                vector_store_id=vector_store_id,
                file_id=file_id,
            )
            vector_deletes_total.add(1, {**metric_attrs, "status": "success"})
    ...
    async def health(self) -> dict[str, HealthResponse]:
        ...
        for provider_id, impl in self.routing_table.impls_by_provider_id.items():  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
            try:
    ...
    async def openai_create_vector_store_file_batch(
        ...
        return await self.routing_table.openai_create_vector_store_file_batch(  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
            vector_store_id=vector_store_id,
            params=params,
        )
    ...
    async def openai_retrieve_vector_store_file_batch(
        ...
        return await self.routing_table.openai_retrieve_vector_store_file_batch(  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
            batch_id=batch_id,
            vector_store_id=vector_store_id,
        )
    ...
    async def openai_list_files_in_vector_store_file_batch(
        ...
        return await self.routing_table.openai_list_files_in_vector_store_file_batch(  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
            batch_id=batch_id,
            vector_store_id=vector_store_id,
            after=after,
            before=before,
    ...
    async def openai_cancel_vector_store_file_batch(
        ...
        return await self.routing_table.openai_cancel_vector_store_file_batch(  # ty: ignore[unresolved-attribute] - routing table is a VectorStoresRoutingTable at runtime
            batch_id=batch_id,
            vector_store_id=vector_store_id,
        )
