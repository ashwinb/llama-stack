...
def prompt_for_config(config_type: type[BaseModel], existing_config: BaseModel | None = None) -> BaseModel:
    ...
    for field_name, field in config_type.__fields__.items():
        ...
        if is_basemodel_without_fields(field_type):
            config_data[field_name] = field_type()  # ty: ignore[call-non-callable]  # guarded by is_basemodel_without_fields check
            continue
        ...
        if inspect.isclass(field_type) and issubclass(field_type, Enum):
            ...
            while True:
                ...
                try:
                    ...
                    validated_value = manually_validate_field(config_type, field, value)  # ty: ignore[invalid-argument-type]  # field used as key name at runtime
        ...
        if is_optional(field_type) and can_recurse(get_non_none_type(field_type)):
            ...
            config_data[field_name] = prompt_for_config(
                field_type,  # ty: ignore[invalid-argument-type]  # guarded by can_recurse check above
                existing_value,
            )
        else:
                ...
                if user_input == "":
                    ...
                    try:
                        ...
                        # Convert the input to the correct type
                            ...
                            value = field_type(user_input)  # ty: ignore[call-non-callable]  # field_type is a concrete type at this point
