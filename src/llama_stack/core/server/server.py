...
def create_dynamic_typed_route(func: Any, method: str, route: str) -> Callable[..., Any]:
    ...
    async def route_handler(request: Request, **kwargs: Any) -> Any:
        ...
        try:
            if is_streaming:
                ...
                gen: Any = preserve_contexts_async_generator(sse_generator(func(**kwargs)), context_vars)  # ty: ignore[invalid-argument-type]
    ...
    route_handler.__signature__ = sig.replace(parameters=new_params)  # ty: ignore[unresolved-attribute]
