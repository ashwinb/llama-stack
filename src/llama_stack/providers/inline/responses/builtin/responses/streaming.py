...
class StreamingResponseOrchestrator:
    ...
    async def create_response(self) -> AsyncIterator[OpenAIResponseObjectStream]:
        ...
        if self.ctx.tool_choice and len(self.ctx.chat_tools) > 0:  # ty: ignore[invalid-argument-type] # chat_tools is checked non-None by tool_choice guard
            processed_tool_choice = await _process_tool_choice(
                self.ctx.chat_tools,  # ty: ignore[invalid-argument-type] # chat_tools is checked non-None above
        ...
        try:
            while True:
                ...
                if allowed_tool_names is not None:
                    effective_tools = [
                        tool
                        for tool in self.ctx.chat_tools  # ty: ignore[not-iterable] # chat_tools is checked non-None above
                ...
                params = OpenAIChatCompletionRequestWithExtraBody(
                    ...
                    safety_identifier=self.safety_identifier,
                    service_tier=self.service_tier,  # ty: ignore[invalid-argument-type] # service_tier str is compatible with ServiceTier enum
    ...
    async def _process_streaming_chunks(
        ...
        yield ChatCompletionResult(
            ...
            logprobs=OpenAIChoiceLogprobs(content=chat_response_logprobs) if chat_response_logprobs else None,  # ty: ignore[invalid-argument-type] # logprobs type mismatch between wrapper and inner type
    ...
    def _build_chat_completion(self, result: ChatCompletionResult) -> OpenAIChatCompletion:
        ...
        assistant_message = OpenAIChatCompletionResponseMessage(
            ...
            tool_calls=tool_calls,  # ty: ignore[invalid-argument-type] # list[OpenAIChatCompletionToolCall] is subset of expected union
        )
        return OpenAIChatCompletion(
            ...
            choices=[
                OpenAIChoice(
                    ...
                    index=0,
                    logprobs=result.logprobs,  # ty: ignore[invalid-argument-type] # logprobs type mismatch between wrapper and inner type
                )
            ],
    ...
    async def _process_new_tools(
        ...
        for input_tool in tools:
            if input_tool.type == "function":
                ...
                async for stream_event in self._process_mcp_tool(input_tool, output_messages):  # ty: ignore[invalid-argument-type] # input_tool.type == "mcp" narrows type at runtime
                    yield stream_event
            else:
    ...
    async def _process_mcp_tool(
        ...
        mcp_tool = await resolve_mcp_connector_id(mcp_tool, self.connectors_api)  # ty: ignore[invalid-argument-type] # connectors_api is checked non-None before MCP tool processing
        ...
        try:
            ...
            with tracer.start_as_current_span("list_mcp_tools", attributes=attributes):  # ty: ignore[invalid-argument-type] # attributes dict is compatible with expected Mapping
                tool_defs = await list_mcp_tools(
                    endpoint=mcp_tool.server_url,  # ty: ignore[invalid-argument-type] # server_url is resolved non-None after resolve_mcp_connector_id
        ...
        match tool_choice:
            case OpenAIResponseInputToolChoiceCustomTool():
                if tool_name and tool_name not in chat_tool_names:
                    ...
                    return None
                return OpenAIChatCompletionToolChoiceCustomTool(name=tool_name)  # ty: ignore[invalid-argument-type] # tool_name is checked non-None by guard above

            case OpenAIResponseInputToolChoiceFunctionTool():
                if tool_name and tool_name not in chat_tool_names:
                    logger.warning("Tool not found in chat tools", tool_name=tool_name)
                    return None
                return OpenAIChatCompletionToolChoiceFunctionTool(name=tool_name)  # ty: ignore[invalid-argument-type] # tool_name is checked non-None by guard above
            ...
            case OpenAIResponseInputToolChoiceMCPTool():
                tool_choice = convert_mcp_tool_choice(  # ty: ignore[invalid-assignment] # tool_choice is reassigned to intermediate dict type before conversion below
                    chat_tool_names,
