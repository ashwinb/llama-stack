...
class StreamingResponseOrchestrator:
        ...
        try:
            while True:
                ...
                if allowed_tool_names is not None:
                    effective_tools = [
                        tool
                        for tool in (self.ctx.chat_tools or [])
                ...
                params = OpenAIChatCompletionRequestWithExtraBody(
                    ...
                    tools=effective_tools,  # ty: ignore[invalid-argument-type]  # Pydantic models are dict-compatible
                    ...
                    safety_identifier=self.safety_identifier,
                    service_tier=self.service_tier,  # ty: ignore[invalid-argument-type]  # service_tier stored as str after enum conversion
    ...
    async def _process_streaming_chunks(
        ...
        yield ChatCompletionResult(
            ...
            logprobs=OpenAIChoiceLogprobs(content=chat_response_logprobs) if chat_response_logprobs else None,  # ty: ignore[invalid-argument-type]  # OpenAIChoiceLogprobs wraps list[OpenAITokenLogProb]
    ...
    def _build_chat_completion(self, result: ChatCompletionResult) -> OpenAIChatCompletion:
        ...
        assistant_message = OpenAIChatCompletionResponseMessage(
            ...
            tool_calls=tool_calls,  # ty: ignore[invalid-argument-type]  # OpenAIChatCompletionToolCall is subtype
        )
        return OpenAIChatCompletion(
            ...
            choices=[
                OpenAIChoice(
                    ...
                    index=0,
                    logprobs=result.logprobs,  # ty: ignore[invalid-argument-type]  # OpenAIChoiceLogprobs wraps list[OpenAITokenLogProb]
                )
            ],
    ...
    async def _process_new_tools(
        ...
        for input_tool in tools:
            if input_tool.type == "function":
                ...
                async for stream_event in self._process_mcp_tool(input_tool, output_messages):  # ty: ignore[invalid-argument-type]  # narrowed by type check above
                    yield stream_event
            else:
    ...
    async def _process_mcp_tool(
        ...
        mcp_tool = await resolve_mcp_connector_id(mcp_tool, self.connectors_api)  # ty: ignore[invalid-argument-type]  # connectors_api is set when MCP tools are used
        ...
        try:
            ...
            with tracer.start_as_current_span("list_mcp_tools", attributes=attributes):  # ty: ignore[invalid-argument-type]  # tracer attributes typing
                tool_defs = await list_mcp_tools(
                    endpoint=mcp_tool.server_url,  # ty: ignore[invalid-argument-type]  # server_url may be None at type level but required at runtime
            ...
            for t in tool_defs.data:
                ...
                if not always_allowed or t.name in always_allowed:
                    ...
                    if self.ctx.chat_tools is None:
                        self.ctx.chat_tools = []
                    self.ctx.chat_tools.append(openai_tool)  # dict matches ChatCompletionToolParam TypedDict
    ...
    async def _reuse_mcp_list_tools(
        ...
        for t in original.tools:
            ...
            if self.ctx.chat_tools is None:
                self.ctx.chat_tools = []
            self.ctx.chat_tools.append(openai_tool)  # ty: ignore[invalid-argument-type]  # dict matches ChatCompletionToolParam TypedDict
        ...
        match tool_choice:
            case OpenAIResponseInputToolChoiceCustomTool():
                if tool_name and tool_name not in chat_tool_names:
                    ...
                    return None
                return OpenAIChatCompletionToolChoiceCustomTool(name=tool_name)  # ty: ignore[invalid-argument-type]  # tool_name is str after truthy check

            case OpenAIResponseInputToolChoiceFunctionTool():
                if tool_name and tool_name not in chat_tool_names:
                    logger.warning("Tool not found in chat tools", tool_name=tool_name)
                    return None
                return OpenAIChatCompletionToolChoiceFunctionTool(name=tool_name)  # ty: ignore[invalid-argument-type]  # tool_name is str after truthy check
            ...
            case OpenAIResponseInputToolChoiceMCPTool():
                tool_choice = convert_mcp_tool_choice(  # ty: ignore[invalid-assignment]  # reassigned to intermediate result
                    chat_tool_names,
