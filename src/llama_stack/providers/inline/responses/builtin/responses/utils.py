    ...
    if isinstance(input, list):
        ...
        for input_item in input:
            if isinstance(input_item, OpenAIResponseInputFunctionToolCallOutput):
                ...
                messages.append(message_type(content=content))  # ty: ignore[invalid-argument-type]  # dynamic message type call with varying content expectations
...
async def convert_response_text_to_chat_response_format(
    ...
    if text.format["type"] == "json_schema":
        ...
        schema_name: str = text.format["name"]  # ty: ignore[invalid-assignment]  # asserted non-None above
...
def is_function_tool_call(
    ...
    if not tool_call.function:
        return False
    for t in tools:
        if t.type == "function" and t.name == tool_call.function.name:  # ty: ignore[unresolved-attribute]  # name exists on function tools
            return True
    return False
...
async def run_guardrails(safety_api: Safety | None, messages: str, guardrail_ids: list[str]) -> str | None:
    ...
    shields_list = await safety_api.routing_table.list_shields()  # ty: ignore[unresolved-attribute]  # runtime-injected routing_table attribute
