    ...
    if isinstance(input, list):
        ...
        for input_item in input:
            if isinstance(input_item, OpenAIResponseInputFunctionToolCallOutput):
                ...
                messages.append(message_type(content=content))  # type: ignore[call-arg,arg-type]  # ty: ignore[invalid-argument-type]  # dynamic message type call with varying content types
...
async def convert_response_text_to_chat_response_format(
    ...
    if text.format["type"] == "json_schema":
        ...
        schema_name: str = text.format["name"]  # type: ignore[assignment]  # ty: ignore[invalid-assignment]  # asserted non-None above
...
def is_function_tool_call(
    ...
    if not tool_call.function:
        return False
    for t in tools:
        if t.type == "function" and t.name == tool_call.function.name:  # ty: ignore[unresolved-attribute]  # name exists on function tool type, guarded by type check
            return True
    return False
...
async def run_guardrails(safety_api: Safety | None, messages: str, guardrail_ids: list[str]) -> str | None:
    ...
    shields_list = await safety_api.routing_table.list_shields()  # type: ignore[attr-defined]  # ty: ignore[unresolved-attribute]  # routing_table available at runtime via API routing
...
def convert_mcp_tool_choice(
    ...
    if tool_name:
        if tool_name not in chat_tool_names:
            return None  # ty: ignore[invalid-return-type]  # None returned intentionally when tool not found
        return {"type": "function", "function": {"name": tool_name}}  # ty: ignore[invalid-return-type]  # nested dict is valid at runtime
        ...
        if not tool_names:
            return None  # ty: ignore[invalid-return-type]  # None returned intentionally when no tools found
        matching_tools = [{"type": "function", "function": {"name": tool_name}} for tool_name in tool_names]
        return matching_tools  # ty: ignore[invalid-return-type]  # nested dict is valid at runtime
    return []
