...
import httpx
import litellm  # ty: ignore[unresolved-import]  # optional third-party dependency
...
class SambaNovaSafetyAdapter(ShieldToModerationMixin, Safety, ShieldsProtocolPrivate, NeedsRequestProviderData):
    ...
    async def register_shield(self, shield: Shield) -> None:
        ...
        provider_id = shield.provider_resource_id or ""
        if (
            "guard" not in provider_id.lower()
            or provider_id.split("sambanova/")[-1] not in self.environment_available_models
        ):
            logger.warning(
